...
class _ScientificCalculatorState extends State<ScientificCalculator> {
  ...
  void _handleModularSign() {
    if (result != '') {
      firstOperand = (double.parse(result) / 100).toString();
    } else if (operators != '') {
      _handleModularInOperators();
    } else {
      if (firstOperand != "") {
  ...
  void _handleDecimalPoint() {
    if (result != '') _clear();
    if (operators != '') {
      _handleDecimalPointInOperators();
    } else {
      _handleDecimalPointInFirstOperand();
    }
  }

  void _handleDecimalPointInOperators() {
    if (!secondOperand.toString().contains(".")) {
      secondOperand == "" ? secondOperand = "." : secondOperand += ".";
    }
  }

  void _handleDecimalPointInFirstOperand() {
    if (!firstOperand.toString().contains(".")) {
      firstOperand == "" ? firstOperand = "." : firstOperand += ".";
    }
  }

  void _handleOperator(value) {
    if (firstOperand == '0') {
  ...
  void _scientificOperands(value) {
    setState(() {
      ...
      if (value == DECIMAL_POINT_SIGN) {
        _handleDecimalPointInScientific();
      }
      equation == ZERO ? equation = value : equation += value;
    });
  }

  void _handleDecimalPointInScientific() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Disneh Calci', style: TextStyle(fontFamily: 'MoonbrightDemo', fontSize: 50)),
        ...
        child: Column(
          children: <Widget>[
            ...
            Keyboard(
              ...
              onTap: (String buttonText) => scientificKeyboard
                    ? _scientificOperands(buttonText)
                    : _simpleOperands(buttonText),
            ),
          ],
